...
st.caption("Ver 9.6 | BTC-KRW → BTC-USD × USD/KRW 방식으로 변경 | 2010년~ 전체 기간")

@st.cache_data(ttl=3600)
def fetch_data_final():
    d = {}
    
    # [A] BTC & DOGE - USD 기준 + USD/KRW 환율로 KRW 변환
    try:
        btc_usd = yf.download("BTC-USD", period="max", progress=False)['Close']
        usd_krw = yf.download("KRW=X", period="max", progress=False)['Close']
        
        # 환율 데이터가 부족하면 최근 값 forward fill + 마지막 값으로 채움
        usd_krw = usd_krw.reindex(btc_usd.index).ffill().bfill()
        
        btc_krw = btc_usd * usd_krw
        d['btc'] = btc_krw.tz_localize(None) if not btc_krw.empty else pd.Series(dtype=float)
    except Exception as e:
        st.warning(f"BTC-USD 또는 KRW=X 로드 실패: {e}")
        d['btc'] = pd.Series(dtype=float)

    try:
        doge_usd = yf.download("DOGE-USD", period="max", progress=False)['Close']
        doge_krw = doge_usd * usd_krw.reindex(doge_usd.index).ffill().bfill()
        d['doge'] = doge_krw.tz_localize(None) if not doge_krw.empty else pd.Series(dtype=float)
    except Exception as e:
        st.warning(f"DOGE-USD 로드 실패: {e}")
    ...
    try:
        ...
        d['nasdaq'] = close.tz_localize(None) if isinstance(close.index, pd.DatetimeIndex) else pd.Series(dtype=float)
    except:
        d['nasdaq'] = pd.Series(dtype=float)

    # [D] Difficulty (기존 JSON)
    try:
...
if not raw['btc'].empty and isinstance(raw['btc'].index, pd.DatetimeIndex):
    ...
    if not btc_v.empty:
        ...
        btc_max_dynamic = 200_000_000_000
    ...
    fig = go.Figure(
        layout=go.Layout(
            ...
            yaxis2=dict(
                title=dict(text="BTC (KRW, approx)", font=dict(color="#00FFEE", size=15)),
    ...
    if not btc_v.empty:
        fig.add_trace(go.Scatter(x=btc_v.index, y=btc_v, name="BTC (-90d, approx KRW)",
    ...
    st.success("✅ 데이터 로드 완료 (BTC/DOGE는 USD×KRW 환율 기준 근사치)")

else:
    st.error("❌ BTC 데이터 로드에 실패했습니다.")
    st.info("• yfinance 연결 상태를 확인해주세요\n• Streamlit Cloud라면 재배포를 시도해보세요")
